...
public abstract class Flight implements Comparable<Flight> {
    ...
    private Set<Passenger> passengers = new HashSet<>(); //when there is a new flight
    private Aircraft vessel;

    public Flight(String flightID, String origin,
                  String destination, DateTime departure, DateTime arrival) {
        setArrival(arrival);
    ...
    public Flight(Flight f) {
        ...
        setDeparture(new DateTime(f.arrival));
        setDestination(destination);
        setFlightID(flightID);
        setOrigin(origin);
    ...
    public DateTime getDeparture() { //when it is defensive we should do like deep
        // copy contructor meaning the class and then the bracket
        return new DateTime(departure);
    }

    public abstract int getBonusMiles(); //when a method is abrtractthat means that
    // it has been
    // implemented
    // in another class

    public int compareTo(Flight o) {
        //for integer Integer.compare(htis.flightID, o.flightID)
        //for double double.compare(this.flightId, o.flightId)
        return this.flightID.compareTo(o.flightID); //String compareTO
    }
    ...
    public String toString() {
        return String.format(
                "%5s from %3.3S (%s) to %3.3S (%s)" + " with a crew of %d and %d " +
                        "passengers "
    ...
    public boolean add(Staff staff) {
        return crew.add(staff); //when it is a list we should do List.contain
        // ourselves
    ...
    public boolean readyToBoard() {
        return crew.size() > 0;  //instead of if true and false
    }

    public boolean boardingCompleted() { ////kheili mohem
